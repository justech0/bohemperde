...
const Products: React.FC = () => {
    ...
    const fetchData = async () => {
      ...
      try {
        const [catRes, prodRes] = await Promise.all([
          fetch(`${API_BASE_URL}/categories/list.php`),
          fetch(`${API_BASE_URL}/products/list.php`)
        ]);
  ...
  const filteredProducts = products.filter(p => {
    ...
    const productCategory = categories.find(c => String(c.id) === String(p.category_id));
  ...
  return (
    <div className="pt-32 pb-20 container mx-auto px-4 md:px-8 min-h-screen bg-bohem-light dark:bg-black transition-colors duration-300">
            ...
            >
              ...
              {categories.map(cat => (
                <button
                  ...
                  className={`px-6 py-2.5 text-sm font-medium whitespace-nowrap rounded-full transition-all border ${selectedCat === String(cat.id) ? 'bg-bohem-dark dark:bg-white text-white dark:text-black border-bohem-dark dark:border-white shadow-md' : 'bg-bohem-paper dark:bg-zinc-900 text-bohem-text dark:text-zinc-400 border-bohem-stone dark:border-zinc-700 hover:border-bohem-gold dark:hover:border-white hover:text-bohem-gold dark:hover:text-white'}`}
                >
                  {cat.name}
                </button>
              ))}
      ...
      {loading && <p className="text-bohem-text dark:text-zinc-400">Yükleniyor...</p>}
